...
등급(basic/plus/premium)과 **무관한 별개 설정**이라 premium 으로 올려도 그대로다.

대시보드에서 손으로 올리면 이 값을 매번 기억해야 하고, 한 번 틀리면 **재인코딩**
말고는 되돌릴 방법이 없다. 그래서 기본값을 코드에 박는다.

**왜 `1080p` 인가**(2026-08-04 확정): 촬영 원본이 1920×1080 이다. 4K 로 올린 줄
알았던 파일들을 Mux 자산 정보로 확인하니 전부 `resolution_tier: 1080p` 였고,
`2160p` 를 요청해도 HD 로 나왔다 — **Mux 는 업스케일하지 않는다.**
상한을 원본보다 높게 열어 두면 나중에 4K 소스가 하나 섞여 들어올 때 인코딩·저장이
조용히 4배가 된다. 촬영이 4K 로 바뀌면 그때 이 값을 올린다.
