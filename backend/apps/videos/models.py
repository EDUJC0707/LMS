...
class MakeupGrant(models.Model):
    ...
    체크(`관리자체크`), 학생·학부모가 직접 신청(`학생신청`/`학부모신청`).

    **승인 단계가 없다** (FLOW 3-4, 2026-08-18 — 구 `승인` 값 제거). 지급 조건은
    **신청 + 결석 확인** 둘뿐이고 순서는 상관없다: 신청이 먼저면 출결이 결석으로
    확정될 때(grades.attendance_admin 트리거 ③), 결석이 먼저면 신청받는 그 자리에서
    나간다. 결석했다는 근거가 이미 출결 SSOT 에 있어 사람이 한 번 더 볼 것이 없다.
    (**클리닉은 반대로 승인제**다 — 자리와 시간을 배정하는 일이라 조교가 본다.
     clinic.ClinicRequest 의 `승인배정` 은 이것과 무관한 별개 값집합이다.)

    `신청` 은 **결석이 아직 확정되지 않은 신청**의 상태로 남는다.
