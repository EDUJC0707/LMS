...
**지금 이 자동 설정은 실제로는 안 켜질 수 있다**(2026-08-04). 회의마다 자동으로
켜는 것은 **Business Plus 이상**이고 현 테넌트 `hjcedu.com` 은 Business Standard
다(PRD 8-5 정정 · 미결 8-20). API 는 200 으로 받고 되읽기에도 ON 으로 남지만
회의에서 무시될 가능성이 높다 — 그때는 조교가 회의 중에 손으로 켜야 한다.
**그래도 계속 보낸다**: 무해하고(200), 등급을 올리는 순간 코드를 고치지 않고
바로 켜지며, PRD 8-5 확정안을 코드가 그대로 담고 있는 편이 맞다.
