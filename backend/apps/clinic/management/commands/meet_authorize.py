...
## 이미 끝난 설정 (2026-08-04) — 다시 조사하지 말 것

계정 `hjcedu@hjcedu.com`(Workspace, `hjcedu.com`) · Cloud 프로젝트
`peak-stream-504505-b0` · OAuth 클라이언트 유형 **데스크톱 앱** ·
동의 화면 대상 **`내부`**.

대상이 `내부`라서 **갱신 토큰은 만료되지 않고 앱 검증도 필요 없다** — 아래
2번의 갈림길은 이미 왼쪽으로 결정됐다. 값 3개는 `backend/.env` 에 있고,
실계정으로 스페이스 생성·`accessType: OPEN` 되읽기까지 확인했다.

아래 순서는 **토큰을 잃어버렸거나 계정을 바꿀 때** 다시 밟는 절차다.
