...
# 창구 끝의 요일 — 모델 요일 축(0=일…6=토)의 화요일.
WINDOW_END_WEEKDAY = 2
...
def booking_window_end(exam_date):
    """③ 창구 끝 — **받을 수 있는 마지막 클리닉 날짜**.
    시험일 다음에 오는 첫 **화요일**(시험일이 화요일이면 7일 뒤).

    2026-07-30 사용자 확정. 창구 끝을 정하는 **유일한 지점** —
    availability·create·change 가 전부 이 함수를 부른다.

    ## 이 함수가 정하지 않는 것 — 신청 마감

    신청 마감은 여기가 아니라 `_check_date_open` 이 정한다. **클리닉 날짜마다
    그 전날이 마감**이라 마감일은 하나로 고정되지 않고 계속 움직인다
    (사용자 원문: "신청마감은 계쏙 바뀌지 / 일요일 클리닉은 토요일까지").
    화요일 클리닉이면 월요일이 마감이고, 금요일 클리닉이면 목요일이 마감이다.

    처음에 이 둘을 "월요일까지"라는 한 문장으로 뭉쳐 읽어 창구 끝을 월요일로
    잡았다가 하루 어긋났다(2026-07-30 정정). **월요일은 화요일 클리닉의
    신청 마감이지 창구 끝이 아니다.**
    ...
    판정은 시험 당일이 될 수도, 며칠 뒤가 될 수도 있다 — 날짜로 못 박지 않는
    이유다.
