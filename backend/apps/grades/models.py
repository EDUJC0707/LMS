...
class Attendance(models.Model):
    ...
    **계약: 이 표가 출결의 단일 원천(SSOT)이다.** 담임이 입력한 status 한 번이
    ...
    반드시 이 표를 참조한다. **"안 왔다" 는 담임이 `결석` 을 찍은 것뿐이다** —
    레코드가 없는 것과 `미입력` 레코드는 같은 뜻이고(아직 아무도 안 봤다) 거기서는
    아무것도 나가지 않는다(FLOW 3-4).

    **status 값집합 5종** (2026-08-18 `미입력` 추가 — 2026-07-29 4종 개정).
    결석 3종은 "왔는가"가 아니라 **보강이 어떻게 됐는가**로 갈린다:

    | 값 | 뜻 | 파생 트리거 |
    |---|---|---|
    | `미입력` | **아직 안 봤다** — 레코드 없음과 같은 뜻 | 없음(있던 지급·대기열은 회수) |
    ...
    결석 셋은 담임이 출결 화면에서 직접 찍는다 — `결석(동보)`·`결석(현보)`도 별도
    신청 절차 없이 출결 입력이 곧 확정이다(사용자 지시 "출결은 둘다 찍는거야").
    `미입력` 은 찍은 것을 **해제**했을 때 남는 값이라 결석이 아니다 — "안 왔다"가
    아니라 "모른다"이므로 ABSENT_STATUSES 에 넣지 않는다.
