...
#: 자식 하나가 이만큼(KB) 쓰면 태스크를 마치고 스스로 교체된다. 머신이 512MB 인데
#: OMR 은 장마다 2400x1700 배열을 만든다 — 누수가 있으면 워커가 아니라 **머신이**
#: OOM 으로 죽고, 그때는 웹까지 같이 재시작된다.
CELERY_WORKER_MAX_MEMORY_PER_CHILD = 300 * 1024
#: 1 vCPU 에 자식 둘. 하나는 늘 CPU 를 쥐고 하나는 I/O(스토리지·DB) 를 기다린다.
#: 기본값(cpu_count)에 맡기면 머신 크기를 바꿀 때 조용히 따라 변한다.
