...
import { Badge, Card, ErrorState, Loading, Table } from "../../components";
...
export default function StudentGradesPage() {
  ...
  if (grades.loading) {
    return <Loading label="성적을 불러오는 중…" />;
  }

  if (grades.error || !grades.data) {
    return <ErrorState description={grades.error ?? undefined} onRetry={grades.reload} />;
  }

  // 옛 페이지 설명(이름 · 원번 · 학교)은 지웠다 — 이름은 계정 칩이 말하고,
  // 원번·학교는 인쇄되는 성적표 상세에만 필요하다.
  const { exams, trend } = grades.data;
    ...
    <div className="ui-stack">
      {taken.length >= 2 ? (
        <Card title="점수 추이" aside={`${taken.length}회 응시`}>
          ...
          />
          {delta !== null && latest && (
            <p className="st-note" style={{ marginTop: "var(--space-lg)" }}>
              직전 회차보다{" "}
              {delta === 0
                ? "점수가 같습니다"
