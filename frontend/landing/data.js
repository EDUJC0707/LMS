...
/* 히어로 배경 후보. 디버그 패널(⌘⌥D) **배경** 그룹에 그대로 뜬다.
   원본 PNG(41MB, 최대 4000x3271)는 그대로 못 쓴다 — 폭 3200 webp 로 구웠다(2.1MB).
   2560 으로 구웠다가 올렸다: 레티나(DPR2)에서 carina 가 1.21배 확대돼 그만큼 물렀다.
   3200 이면 필요한 3107px 을 덮는다. 품질도 82 → 90(압축 손실 PSNR 38~45dB).
   **-1600 벌을 같이 굽는다.** 캔버스라 srcset 이 안 걸려서 field.js 가 직접 고른다
   (화면 폭 × DPR ≤ 1600 이면 작은 벌). 안 그러면 모바일이 0.9MB 를 통째로 받는다.
