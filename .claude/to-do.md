...
- [ ] **API 잔여 완주 (사용자 지시: 나머지 다, 중간 커밋)**
  - [ ] 슬라이스 5: 성적·성적표 조회(학생·학부모·관리자, PRD 3.2.1 구성)
  - [ ] 슬라이스 6: 게시판(카테고리 5종 권한·비밀글·댓글)
  - [ ] 슬라이스 7: 워크북 업로드·열람(OCR 수용 구조, 인식 엔진 제외)
  - [ ] 슬라이스 8: 관리자 운영 — 권한 매트릭스(대표 전용)·계정 일괄 발급(SMS 제외)·클리닉 배정·상담 대기열·알림 내역
  - 외부 연동 제외: 결제선생 API·알림톡 발송(7/29)·DRM 연동(업체 대기)·OMR 인식 엔진
